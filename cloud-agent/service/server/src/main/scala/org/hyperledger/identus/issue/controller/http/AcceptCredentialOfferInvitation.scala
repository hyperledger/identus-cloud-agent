package org.hyperledger.identus.issue.controller.http

import org.hyperledger.identus.api.http.Annotation
import sttp.tapir.Schema
import sttp.tapir.Schema.annotations.{description, encodedExample}
import zio.json.{DeriveJsonDecoder, DeriveJsonEncoder, JsonDecoder, JsonEncoder}
import AcceptCredentialOfferInvitation.annotations

case class AcceptCredentialOfferInvitation(
    @description(annotations.invitation.description)
    @encodedExample(annotations.invitation.example)
    invitation: String
)

object AcceptCredentialOfferInvitation {

  object annotations {
    object invitation
        extends Annotation[String](
          description = "The base64-encoded raw invitation.",
          example =
            ""
        )
  }

  given encoder: JsonEncoder[AcceptCredentialOfferInvitation] =
    DeriveJsonEncoder.gen[AcceptCredentialOfferInvitation]

  given decoder: JsonDecoder[AcceptCredentialOfferInvitation] =
    DeriveJsonDecoder.gen[AcceptCredentialOfferInvitation]

  given schema: Schema[AcceptCredentialOfferInvitation] = Schema.derived

}
