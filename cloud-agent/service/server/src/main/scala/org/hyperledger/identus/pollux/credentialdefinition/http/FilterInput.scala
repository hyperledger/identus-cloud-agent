...
case class FilterInput(
  ...
  def toDomain(resolutionMethod: ResourceResolutionMethod = ResourceResolutionMethod.HTTP) = CredentialDefinition.Filter(author, name, version, tag, resolutionMethod)
}
