...
object PrismDIDSpec extends ZIOSpecDefault {
  ...
  private val createOperationDummy = PublishedDIDOperation.Create(
  ...
  private val prismDIDV1Suite =
    suite("PrismDIDV1")(
      test("render correct DID syntax") {
        val prismDID = PrismDIDV1.fromCreateOperation(createOperationDummy)
      ...
      test("DID syntax is the same when converted to standard DID") {
        ...
        check(ledgerNameGen) { ledgerName =>
          val op = createOperationDummy.copy(storage = DIDStorage.Cardano(ledgerName))
  ...
  private val longFormPrismDIDV1Suite =
    suite("LongFormPrismDIDV1")(
      test("render correct DID syntax") {
        val longFormPrismDID = LongFormPrismDIDV1.fromCreateOperation(createOperationDummy)
