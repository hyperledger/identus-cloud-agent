...
PREFIX = lib
LIBEXT = so
LIBNAME = $(PREFIX)anoncreds-shim.$(LIBEXT)

export MACOSX_DEPLOYMENT_TARGET=10.5

CCACHE := $(strip $(realpath $(shell which ccache 2> /dev/null)))

SHIM_SRCS = $(wildcard $(SRC_DIR)/*.c $(SRC_DIR)/*/*.c)
SHIM_OBJS := $(patsubst $(SRC_DIR)/%.c, $(SHIM_BUILD_DIR)/%.o, $(SHIM_SRCS))

ANONCREDS_RS_INCLUDE=${SRC_DIR}
...
LDFLAGS += $(SOFLAGS)
LDGLAGS += -rpath ${RT_LOCATION_ANONCREDS_SO}

IFLAGS = -I"$(BUILD_DIR)" -I"${ANONCREDS_RS_INCLUDE}"
...
LIBSHIM = $(SHIM_BUILD_DIR)/$(LIBNAME)

all:	$(LIBSHIM)

$(SHIM_BUILD_DIR)/%.o : $(SRC_DIR)/%.c
	@mkdir -p $(@D)
	$(CCACHE) $(CC) $(CFLAGS) -c $< -o "$@"

$(LIBSHIM):  $(SHIM_OBJS)
	$(CC) -o $@ $(LDFLAGS) $(SHIM_OBJS) -lm
