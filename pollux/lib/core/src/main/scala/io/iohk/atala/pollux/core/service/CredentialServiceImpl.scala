...
object CredentialServiceImpl {
  val layer: URLayer[
    CredentialRepository & CredentialStatusListRepository & DidResolver & URIDereferencer & GenericSecretStorage & CredentialDefinitionService &
  ...
  private[this] def createJwtIssuer(
    ...
    for {
      ...
      ecKeyPair <- managedDIDService
        ...
        .debug("issuing key id " + issuingKeyId)
        .debug("Getting did - " + jwtIssuerDID.asCanonical)
