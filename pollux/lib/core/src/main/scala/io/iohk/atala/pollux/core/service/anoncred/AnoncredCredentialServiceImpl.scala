package io.iohk.atala.pollux.core.service.anoncred
...
import io.iohk.atala.pollux.core.repository.anoncred.AnoncredCredentialRepository
...
object AnoncredCredentialServiceImpl {
  val layer: URLayer[IrisServiceStub & AnoncredCredentialRepository[Task] & DidResolver & URIDereferencer, AnoncredCredentialService] =
    ZLayer.fromFunction(AnoncredCredentialServiceImpl(_, _, _, _))
...
private class AnoncredCredentialServiceImpl(
                                             irisClient: IrisServiceStub,
                                             anoncredCredentialRepository: AnoncredCredentialRepository[Task],
                                             didResolver: DidResolver,
                                             uriDereferencer: URIDereferencer,
                                             maxRetries: Int = 5 // TODO move to config
) extends AnoncredCredentialService {

  import AnoncredCredentialServiceImpl.*
  ...
  override def getIssueCredentialRecords(
    ...
    for {
      records <- anoncredCredentialRepository
  ...
  override def getIssueCredentialRecordByThreadId(
    ...
    for {
      record <- anoncredCredentialRepository
  ...
  override def createIssueCredentialRecord(
    ...
    for {
      _ <- maybeSchemaId match
        ...
        case None =>
          ZIO.unit
      attributes <- AnoncredCredentialService.convertJsonClaimsToAttributes(claims)
      offer <- ZIO.succeed(
      ...
      count <- anoncredCredentialRepository
  ...
  override def receiveCredentialRequest(
    ...
    for {
      ...
      _ <- anoncredCredentialRepository
  ...
  private[this] def getRecordWithState(
    ...
    for {
      maybeRecord <- anoncredCredentialRepository
        .getIssueCredentialRecord(recordId)
        .mapError(RepositoryError.apply)
      record <- ZIO
  ...
  override def createCredentialPayloadFromRecord(
    ...
    val credential = for {
      ...
      claims <- AnoncredCredentialService.convertAttributesToJsonClaims(preview.attributes)
      ...
      jwtPresentation <- validateRequestCredentialDataProof(maybeOfferOptions, requestJwt).tapBoth(
        error =>
          ZIO.logErrorCause("JWT Presentation Validation Failed!!", Cause.fail(error)) *> anoncredCredentialRepository
            .updateCredentialRecordProtocolState(
              record.id,
