package io.iohk.atala.pollux.core.repository.anoncred
...
class AnoncredCredentialRepositoryInMemory(
...
object AnoncredCredentialRepositoryInMemory {
  val maxRetries = 5 // TODO Move to config
  val layer: ULayer[AnoncredCredentialRepository[Task]] = ZLayer.fromZIO(
    Ref
      .make(Map.empty[DidCommID, IssueCredentialRecord])
      .map(AnoncredCredentialRepositoryInMemory(_, maxRetries))
  )
}
