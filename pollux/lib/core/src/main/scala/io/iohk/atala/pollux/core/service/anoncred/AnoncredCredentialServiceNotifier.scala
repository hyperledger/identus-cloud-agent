...
import io.iohk.atala.pollux.vc.jwt.PresentationPayload
...
class AnoncredCredentialServiceNotifier(
    svc: AnoncredCredentialService,
...
object AnoncredCredentialServiceNotifier {
  val layer: URLayer[AnoncredCredentialService & EventNotificationService, AnoncredCredentialServiceNotifier] =
    ZLayer.fromFunction(AnoncredCredentialServiceNotifier(_, _))
}
