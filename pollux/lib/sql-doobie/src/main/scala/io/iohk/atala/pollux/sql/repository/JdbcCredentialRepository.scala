...
class JdbcCredentialRepository(xa: Transactor[ContextAwareTask], xb: Transactor[Task], maxRetries: Int)
  ...
  given acRequestMetadataGet: Get[lib.CredentialRequestMetadata] =
    Get[String].map(_.fromJson[lib.CredentialRequestMetadata].getOrElse(???))
  given acRequestMetadataPut: Put[lib.CredentialRequestMetadata] = Put[String].contramap(_.toJson)
  ...
  override def updateWithAnonCredsRequestCredential(
      recordId: DidCommID,
      request: RequestCredential,
      metadata: lib.CredentialRequestMetadata,
      protocolState: ProtocolState
  ): RIO[WalletAccessContext, Int] = {
    val cxnIO =
      sql"""
