...
class JdbcAnoncredCredentialRepository(xa: Transactor[Task], maxRetries: Int)
    extends AnoncredCredentialRepository[Task] {
...
object JdbcAnoncredCredentialRepository {
  val maxRetries = 5 // TODO Move to config
  val layer: URLayer[Transactor[Task], AnoncredCredentialRepository[Task]] =
    ZLayer.fromFunction(new JdbcAnoncredCredentialRepository(_, maxRetries))
}
