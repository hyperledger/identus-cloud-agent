...
/**
 * Optional value for overriding the non-revoked interval in the
 * [`PresentationRequest`] This only overrides the `from` value as a Revocation
 * Status List is deemed valid until the next entry.
 *
...
ErrorCode anoncreds_create_credential(
    ObjectHandle cred_def, ObjectHandle cred_def_private,
    ObjectHandle cred_offer, ObjectHandle cred_request, FfiStrList attr_names,
    FfiStrList attr_raw_values, FfiStrList attr_enc_values, FfiStr rev_reg_id,
    ObjectHandle rev_status_list, const struct FfiCredRevInfo *revocation,
    ObjectHandle *cred_p);

ErrorCode anoncreds_create_credential_definition(
    FfiStr schema_id, ObjectHandle schema, FfiStr tag, FfiStr issuer_id,
    FfiStr signature_type, int8_t support_revocation, ObjectHandle *cred_def_p,
    ObjectHandle *cred_def_pvt_p, ObjectHandle *key_proof_p);
...
ErrorCode anoncreds_create_credential_request(
    FfiStr entropy, FfiStr prover_did, ObjectHandle cred_def,
    FfiStr link_secret, FfiStr link_secret_id, ObjectHandle cred_offer,
    ObjectHandle *cred_req_p, ObjectHandle *cred_req_meta_p);

ErrorCode anoncreds_create_link_secret(const char **link_secret_p);

ErrorCode anoncreds_create_or_update_revocation_state(
    ObjectHandle rev_reg_def, ObjectHandle rev_status_list,
    int64_t rev_reg_index, FfiStr tails_path, ObjectHandle rev_state,
    ObjectHandle old_rev_status_list, ObjectHandle *rev_state_p);

ErrorCode anoncreds_create_presentation(
    ObjectHandle pres_req, struct FfiList_FfiCredentialEntry credentials,
    struct FfiList_FfiCredentialProve credentials_prove,
    FfiStrList self_attest_names, FfiStrList self_attest_values,
    FfiStr link_secret, struct FfiList_ObjectHandle schemas,
    FfiStrList schema_ids, struct FfiList_ObjectHandle cred_defs,
    FfiStrList cred_def_ids, ObjectHandle *presentation_p);

ErrorCode anoncreds_create_revocation_registry_def(
    ObjectHandle cred_def, FfiStr cred_def_id, FfiStr issuer_id, FfiStr tag,
    FfiStr rev_reg_type, int64_t max_cred_num, FfiStr tails_dir_path,
    ObjectHandle *reg_def_p, ObjectHandle *reg_def_private_p);

ErrorCode anoncreds_create_revocation_status_list(
    FfiStr rev_reg_def_id, ObjectHandle rev_reg_def, FfiStr issuer_id,
    int64_t timestamp, int8_t issuance_by_default,
    ObjectHandle *rev_status_list_p);

ErrorCode anoncreds_create_schema(FfiStr schema_name, FfiStr schema_version,
                                  FfiStr issuer_id, FfiStrList attr_names,
                                  ObjectHandle *result_p);

ErrorCode anoncreds_credential_definition_from_json(struct ByteBuffer json,
                                                    ObjectHandle *result_p);

ErrorCode
anoncreds_credential_definition_private_from_json(struct ByteBuffer json,
                                                  ObjectHandle *result_p);

ErrorCode anoncreds_credential_from_json(struct ByteBuffer json,
                                         ObjectHandle *result_p);

ErrorCode anoncreds_credential_get_attribute(ObjectHandle handle, FfiStr name,
                                             const char **result_p);

ErrorCode anoncreds_credential_offer_from_json(struct ByteBuffer json,
                                               ObjectHandle *result_p);

ErrorCode anoncreds_credential_request_from_json(struct ByteBuffer json,
                                                 ObjectHandle *result_p);

ErrorCode
anoncreds_credential_request_metadata_from_json(struct ByteBuffer json,
                                                ObjectHandle *result_p);

ErrorCode anoncreds_encode_credential_attributes(FfiStrList attr_raw_values,
                                                 const char **result_p);
...
ErrorCode anoncreds_key_correctness_proof_from_json(struct ByteBuffer json,
                                                    ObjectHandle *result_p);

void anoncreds_object_free(ObjectHandle handle);

ErrorCode anoncreds_object_get_json(ObjectHandle handle,
                                    struct ByteBuffer *result_p);

ErrorCode anoncreds_object_get_type_name(ObjectHandle handle,
                                         const char **result_p);

ErrorCode anoncreds_presentation_from_json(struct ByteBuffer json,
                                           ObjectHandle *result_p);

ErrorCode anoncreds_presentation_request_from_json(struct ByteBuffer json,
                                                   ObjectHandle *result_p);

ErrorCode
anoncreds_process_credential(ObjectHandle cred, ObjectHandle cred_req_metadata,
                             FfiStr link_secret, ObjectHandle cred_def,
                             ObjectHandle rev_reg_def, ObjectHandle *cred_p);

ErrorCode
anoncreds_revocation_registry_definition_from_json(struct ByteBuffer json,
                                                   ObjectHandle *result_p);

ErrorCode anoncreds_revocation_registry_definition_get_attribute(
    ObjectHandle handle, FfiStr name, const char **result_p);

ErrorCode anoncreds_revocation_registry_definition_private_from_json(
    struct ByteBuffer json, ObjectHandle *result_p);

ErrorCode anoncreds_revocation_registry_from_json(struct ByteBuffer json,
                                                  ObjectHandle *result_p);

ErrorCode anoncreds_revocation_state_from_json(struct ByteBuffer json,
                                               ObjectHandle *result_p);

ErrorCode anoncreds_revocation_status_list_from_json(struct ByteBuffer json,
                                                     ObjectHandle *result_p);

ErrorCode anoncreds_schema_from_json(struct ByteBuffer json,
                                     ObjectHandle *result_p);
...
ErrorCode anoncreds_update_revocation_status_list(
    int64_t timestamp, struct FfiList_i32 issued, struct FfiList_i32 revoked,
    ObjectHandle rev_reg_def, ObjectHandle rev_current_list,
    ObjectHandle *new_rev_status_list_p);

ErrorCode anoncreds_update_revocation_status_list_timestamp_only(
    int64_t timestamp, ObjectHandle rev_current_list,
    ObjectHandle *rev_status_list_p);

ErrorCode anoncreds_verify_presentation(
    ObjectHandle presentation, ObjectHandle pres_req,
    struct FfiList_ObjectHandle schemas, FfiStrList schema_ids,
    struct FfiList_ObjectHandle cred_defs, FfiStrList cred_def_ids,
    struct FfiList_ObjectHandle rev_reg_defs, FfiStrList rev_reg_def_ids,
