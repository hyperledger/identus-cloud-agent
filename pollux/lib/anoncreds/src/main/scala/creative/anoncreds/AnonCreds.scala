...
object AnonCreds {
  ...
  def apply(pathsToSearch: Seq[String],
            libsToLoad: Seq[String] = ClasspathSharedObject.namesOfSharedObjectsToLoad): AnonCreds = {

    val withPathsToSearch = pathsToSearch.foldLeft(LibraryLoader.create(classOf[AnonCreds])) {
...
trait AnonCreds {
  ...
  def shim_anoncreds_encode_credential_attributes(
                                                   ...
                                                   @In count: Int,
                                                   @Out result_p: PointerByReference): ErrorCode
  ...
  def shim_anoncreds_create_credential(
                                   ...
                                   @Out cred_p: PointerByReference): ErrorCode
  ...
  def anoncreds_create_credential_definition(
                                              ...
                                              @Out key_proof_p: PointerByReference): ErrorCode


  def shim_anoncreds_object_get_json(@In handle: ObjectHandle, @Out result_p: PointerByReference): ErrorCode

  def anoncreds_create_credential_offer(schema_id: FfiStr,
                                        cred_def_id: FfiStr,
                                        key_proof: ObjectHandle,
                                        @Out cred_offer_p: ObjectHandle_star): ErrorCode

  def anoncreds_create_credential_request(@In prover_did: FfiStr,
                                          ...
                                          @Out cred_req_meta_p: ObjectHandle_star): ErrorCode

  def anoncreds_create_master_secret(@Out master_secret_p: ObjectHandle_star): ErrorCode


  def anoncreds_create_or_update_revocation_state(@In rev_reg_def: ObjectHandle,
                                                  ...
                                                  @Out rev_state_p: ObjectHandle_star): ErrorCode


  def shim_anoncreds_create_presentation(pres_req: ObjectHandle,
  ...
  def anoncreds_create_revocation_registry_def(cred_def: ObjectHandle,
                                               cred_def_id: FfiStr,
                                               issuer_id: FfiStr,
                                               tag: FfiStr,
                                               rev_reg_type: FfiStr,
                                               ...
                                               @Out reg_def_private_p: ObjectHandle_star): ErrorCode

  def anoncreds_create_revocation_status_list(rev_reg_def_id: FfiStr,
                                              rev_reg_def: ObjectHandle,
                                              timestamp: Long@int64_t,
                                              issuance_by_default: Int @int8_t,
                                              @Out rev_status_list_p: ObjectHandle_star): ErrorCode

  def anoncreds_credential_get_attribute(handle:ObjectHandle,
                                    name: FfiStr,
                                         @Out result_p: PointerByReference): ErrorCode

  def anoncreds_object_get_type_name(handle: ObjectHandle, @Out result_p: PointerByReference): ErrorCode

  def anoncreds_process_credential(cred: ObjectHandle,
                                   cred_req_metadata: ObjectHandle,
                                   master_secret: ObjectHandle,
                                   cred_def: ObjectHandle,
                                   rev_reg_def: ObjectHandle,
                                   @Out cred_p: ObjectHandle_star): ErrorCode

  def anoncreds_revocation_registry_definition_get_attribute(handle: ObjectHandle,
                                                             name: FfiStr,
                                                             @Out result_p: PointerByReference): ErrorCode

  def anoncreds_set_default_logger(): Unit

  def shim_anoncreds_update_revocation_status_list(timestamp: Long@int64_t,
                                                   issued: FfiList_i32, issued_count: Long@size_t,
                                                   revoked: FfiList_i32, revoked_count: Long@int32_t,
                                                   rev_reg_def: ObjectHandle,
                                                   rev_current_list: ObjectHandle,
                                                   @Out new_rev_status_list_p: ObjectHandle_star): ErrorCode

  def anoncreds_update_revocation_status_list_timestamp_only(timestamp: Long@int64_t,
                                                             rev_current_list: ObjectHandle,
                                                             @Out rev_status_list_p: ObjectHandle_star): ErrorCode

  def shim_anoncreds_presentation_request_from_json(@In json: String,
                                               @int64_t json_len: Long,
                                               @Out presentation_req: PointerByReference): ErrorCode

  def shim_anoncreds_verify_presentation(presentation: ObjectHandle,
                                         ...
                                         @Out result_p: StringBuffer): ErrorCode
