...
package object sql {
  ...
  given walletIdGet: Get[WalletId] = Get[String].map(s => WalletId.fromUUID(UUID.fromString(s)))
  given walletIdPut: Put[WalletId] = Put[String].contramap(_.toUUID.toString())
