...
object HttpRoutes {
  ...
  def routes: URIO[
    DIDApi & DIDOperationsApi & DIDAuthenticationApi & IssueCredentialsApi & IssueCredentialsProtocolApi,
    Route
  ] =
    for {
    ...
    } yield didApi.route ~ didOperationsApi.route ~ didAuthApi.route ~ issueCredentialApi.route ~ issueCredentialsProtocolApi.route ~ additionalRoute
