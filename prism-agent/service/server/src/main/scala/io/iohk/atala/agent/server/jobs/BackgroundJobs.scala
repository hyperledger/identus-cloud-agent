  ...
  private[this] def performExchange(
    ...
    val aux = for {
      ...
      _ <- record match {
        ...
        case IssueCredentialRecord(id, _, _, Role.Issuer, _, _, _, Some(awaitConfirmation), CredentialPending, _, _, _, Some(issue)) =>
        ...
        case IssueCredentialRecord(id, _, _, Role.Issuer, _, _, _, Some(false), CredentialGenerated, None, _, _, Some(issue)) =>
          for {
        ...
        case IssueCredentialRecord(id, _, _, Role.Issuer, _, _, _, Some(true), CredentialGenerated, Some(Published), _, _, Some(issue)) =>
          for {
            _ <- sendMessage(issue.makeMessage)
            credentialService <- ZIO.service[CredentialService]
            _ <- credentialService.markCredentialSent(id)
