  ...
  private[this] def performIssueCredentialExchange(record: IssueCredentialRecord) = {
    ...
    val aux = for {
      ...
      _ <- record match {
        ...
        case IssueCredentialRecord(id, _, _, _, _, _, _, _, _, _, _, ProblemReportPending, _, _, _, _, _, _, _, _, _) =>
          ???
        case IssueCredentialRecord(id, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _) => ZIO.unit
      }
    } yield ()
