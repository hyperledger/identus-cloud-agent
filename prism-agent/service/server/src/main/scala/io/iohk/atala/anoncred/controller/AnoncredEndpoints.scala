...
import io.iohk.atala.anoncred.controller.http.*
...
object AnoncredEndpoints {

  private val paginationInput: EndpointInput[PaginationInput] = EndpointInput.derived[PaginationInput]

  val createCredentialOffer: PublicEndpoint[
    (RequestContext, CreateAnoncredRecordRequest),
    ErrorResponse,
    AnoncredRecord,
    Any
  ] =
    endpoint.post
      ...
      .in("issue-anoncred" / "credential-offers")
      .in(jsonBody[CreateAnoncredRecordRequest].description("The credential offer object."))
      .errorOut(basicFailures)
      .out(statusCode(StatusCode.Created))
      .out(jsonBody[AnoncredRecord].description("The issue credential record."))
  ...
  val getCredentialRecords: PublicEndpoint[
    ...
    endpoint.get
      .in(extractFromRequest[RequestContext](RequestContext.apply))
      .in("issue-anoncred" / "records")
      .in(paginationInput)
      .in(query[Option[String]]("thid").description("The thid of a DIDComm communication."))
      .errorOut(basicFailures)
      .out(jsonBody[AnoncredRecordPage].description("The list of issue credential records."))
      .tag("Issue Credentials Protocol")
  ...
  val getCredentialRecord: PublicEndpoint[
    ...
    endpoint.get
      .in(extractFromRequest[RequestContext](RequestContext.apply))
      .in(
        "issue-anoncred" / "records" / path[String]("recordId").description(
  ...
  val acceptCredentialOffer: PublicEndpoint[
    (RequestContext, String, AcceptAnoncredOfferRequest),
    ErrorResponse,
    AnoncredRecord,
    Any
  ] =
    endpoint.post
      ...
      .in("accept-offer")
      .in(jsonBody[AcceptAnoncredOfferRequest].description("The accept credential offer request object."))
      .errorOut(basicFailuresAndNotFound)
      .out(jsonBody[AnoncredRecord].description("The issue credential offer was successfully accepted."))
      .tag("Issue Credentials Protocol")
  ...
  val issueCredential: PublicEndpoint[
    ...
    endpoint.post
      ...
      .errorOut(basicFailuresAndNotFound)
      .out(
        jsonBody[AnoncredRecord].description(
