...
import io.iohk.atala.anoncred.controller.http.AnoncredRecord.annotations
import io.iohk.atala.mercury.model.{AttachmentDescriptor, Base64}
import io.iohk.atala.pollux.core.model.IssueCredentialRecord as PolluxAnoncredRecord //FIXME
...
/** A class to represent an an outgoing response for a created credential offer.
  ...
  */
final case class AnoncredRecord(
...
object AnoncredRecord {

  def fromDomain(domain: PolluxAnoncredRecord): AnoncredRecord =
    AnoncredRecord(
  ...
  given Conversion[PolluxAnoncredRecord, AnoncredRecord] = fromDomain
  ...
  given encoder: JsonEncoder[AnoncredRecord] =
    DeriveJsonEncoder.gen[AnoncredRecord]

  given decoder: JsonDecoder[AnoncredRecord] =
    DeriveJsonDecoder.gen[AnoncredRecord]

  given schema: Schema[AnoncredRecord] = Schema.derived
