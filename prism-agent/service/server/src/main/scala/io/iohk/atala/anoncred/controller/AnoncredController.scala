...
import io.iohk.atala.anoncred.controller.http.{CreateAnoncredRecordRequest, AnoncredRecord, AnoncredRecordPage}
import io.iohk.atala.anoncred.controller.http.AcceptAnoncredOfferRequest
import io.iohk.atala.pollux.core.model.error.CredentialServiceError
import zio.*

trait AnoncredController {
  def createCredentialOffer(request: CreateAnoncredRecordRequest)(implicit
      rc: RequestContext
  ...
  def getCredentialRecord(recordId: String)(implicit rc: RequestContext): IO[ErrorResponse, AnoncredRecord]

  def acceptCredentialOffer(recordId: String, request: AcceptAnoncredOfferRequest)(implicit
      rc: RequestContext
  ): IO[ErrorResponse, AnoncredRecord]

  def issueCredential(recordId: String)(implicit rc: RequestContext): IO[ErrorResponse, AnoncredRecord]

}

object AnoncredController {
